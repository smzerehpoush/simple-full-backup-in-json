...
function createFile() {
  ...
  if (!fs.existsSync(structurePath)) fs.mkdirSync(structurePath);
  if (!fs.existsSync(queryPath)) fs.mkdirSync(queryPath);
  backup();
}
...
function parseTableData(connection, tableName) {
  ...
  connection.query(query, function(error, rows, fields) {
    ...
    wirteJSONToFile(
      `${configuration.mainDirectory}/data/${tableName}.json`,
      tableData
    );
  });
}
